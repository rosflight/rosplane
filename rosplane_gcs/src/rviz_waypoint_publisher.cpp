...
rviz_waypoint_publisher::rviz_waypoint_publisher()
    ...
    rviz_pub_ = this->create_publisher<visualization_msgs::msg::Marker>("rviz/waypoint", 10);
...
void rviz_waypoint_publisher::new_wp_callback(const rosplane_msgs::msg::Waypoint & wp) {
    ...
    rviz_pub_->publish(new_marker);
    rviz_pub_->publish(line_list_);
