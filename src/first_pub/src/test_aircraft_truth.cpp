...
class AircraftTruth : public rclcpp::Node
{
    ...
    private:
        ...
        void topic_callback(const Odometry & msg) const
        {
            ...
            truthState.phi = atan2(2*(gazeboOrient.w * gazeboOrient.x + gazeboOrient.y * gazeboOrient.z), gazeboOrient.w^2 + gazeboOrient.z^2 - gazeboOrient.x^2 - gazeboOrient.y^2)
            truthState.theta = asin(2*(gazeboOrient.w * gazeboOrient.y - gazeboOrient.x * gazeboOrient.z))
            truthState.psi = atan2(2*(gazeboOrient.w * e3 + gazeboOrient.x * gazeboOrient.y), gazeboOrient.w^2 + gazeboOrient.x^2 - gazeboOrient.y^2 - gazeboOrient.z^2)
            ...
            double ur = u - truthState.wn;
            double vr = v - truthState.we;
            double wr = w - wd;

            truthState.va = sqrt(pow(ur, 2.0) + pow(vr, 2.0) + pow(wr, 2.0));
            truthState.chi = atan2(msg.Va*sin(msg.psi), msg.Va*cos(msg.psi));
            truthState.alpha = atan2(wr, ur);
            truthState.beta = asin(vr/msg.Va);
