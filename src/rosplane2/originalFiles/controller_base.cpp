...
controller_base::controller_base() : Node("controller_base")
{
  ...
  command_recieved_ = true; // false; TODO go back to false.
}

void controller_base::actuator_controls_publish()
{
  struct input_s input;
  input.h =100.0; //-vehicle_state_.position[2]; // TODO connect back to the memeber variables.
  input.va = 25.0; //vehicle_state_.Va;
  input.phi = 0.0; //vehicle_state_.phi;
  input.theta = 0.0; //vehicle_state_.theta;
  input.chi = 0.0; //vehicle_state_.chi;
  input.p = 0.0; //vehicle_state_.p;
  input.q = 0.0; //vehicle_state_.q;
  input.r = 0.0; //vehicle_state_.r;
  input.Va_c = 25.0; //controller_commands_.Va_c;
  input.h_c = 100.0; //controller_commands_.h_c;
  input.chi_c = 0.0; //controller_commands_.chi_c;
  input.phi_ff = 0.0; //controller_commands_.phi_ff;
  input.Ts = 0.01f;
...
//    if (internals_pub_.getNumSubscribers() > 0) TODO add back in with internals pub
//    {
...
//  rosplane2::controller_base *cont = new rosplane2::controller_example();
  ...
  rclcpp::spin(std::make_shared<rosplane2::controller_example>()); // TODO figure out the pure virtual function thing.
