...
estimator_base::estimator_base() : Node("estimator_base") {
    ...
    update_timer_ = this->create_wall_timer(std::chrono::milliseconds((int)params_.Ts), std::bind(&estimator_base::update, this));
...
{
  bool has_fix = msg->status.status >= sensor_msgs::msg::NavSatStatus::STATUS_FIX; // Higher values refer to augmented fixes
...
int main(int argc, char **argv)
{
  ...
  rclcpp::spin(std::make_shared<rosplane2::estimator_example>());
