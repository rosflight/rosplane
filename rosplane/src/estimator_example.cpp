...
void estimator_example::estimate(const params_s & params, const input_s & input, output_s & output)
{
  ...
  for (int i = 0; i < N_; i++) {
    ...
    Eigen::MatrixXf A_d = Eigen::MatrixXf::Identity(2, 2) + params.Ts / N_ * A_a_
      + pow(params.Ts / N_, 2) / 2.0 * A_a_ * A_a_;

    Eigen::Matrix<float, 2, 3> G;
    G << 1, sp * tt, cp * tt, 0.0, cp, -sp;

    P_a_ =
      (A_d * P_a_ * A_d.transpose() + (Q_a_ + G * Q_g_ * G.transpose()) * pow(params.Ts / N_, 2));
  }
  // measurement updates
  cp = cosf(xhat_a_(0));
  sp = sinf(xhat_a_(0));
  ...
  C_a_ << 0.0, qhat * Vahat * ct + params.gravity * ct, -params.gravity * cp * ct,
    -rhat * Vahat * st - phat * Vahat * ct + params.gravity * sp * st, params.gravity * sp * ct,
    (qhat * Vahat + params.gravity * cp) * st;
  ...
  for (int i = 0; i < N_; i++) {
    ...
    Eigen::MatrixXf A_d_ = Eigen::MatrixXf::Identity(7, 7) + params.Ts / N_ * A_p_
      + A_p_ * A_p_ * pow(params.Ts / N_, 2) / 2.0;
